...
Category is backend organization and Starlight fallback browse. It is not the primary player journey and should not be used as a substitute for Routes.
...
V1 guidance: author 3–5 high-confidence pages. Schema has no hard cap; Skill must warn when larger.

## `routes[]`

Optional player-facing Route paths. A Route is a **player task path** (`/{hubPath}/routes/{routeId}/`),
