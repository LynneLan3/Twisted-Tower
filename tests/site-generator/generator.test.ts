...
test('13.3 enabled ga4 with privacy emits runtime config and defaults trackOutbound', () => {
	const workspace = copyTemplateWorkspace();
	try {
		installFixture(workspace, 'valid-site');
		installTrustInputs(workspace);
...
test('integration build includes GA4 config only when enabled', { timeout: 180_000 }, () => {
	...
	try {
		...
		assert.match(hubHtml, /"trackOutbound":true/);
		assert.match(overviewHtml, /data-outbound-kind="source"/);
		assert.match(overviewHtml, /data-outbound-kind="evidence"/);
	} finally {
		rmSync(workspace, { recursive: true, force: true });
...
test('integration build passes when generated config includes routes', { timeout: 180_000 }, () => {
	...
	try {
		...
		assert.equal(build.status, 0, build.stdout + build.stderr);
		// V2.2-C2: the formal Route Hub lives under the template hub directory
		// (src/pages/example-game/routes/[routeId].astro). A generated site with a
		// different hub (/fixture-game/) therefore produces no fixture route hubs.
		assert.equal(existsSync(path.join(workspace, 'dist/fixture-game/routes/getting-started/index.html')), false);
		assert.ok(existsSync(path.join(workspace, 'dist/example-game/routes/getting-started/index.html')));
	} finally {
		rmSync(workspace, { recursive: true, force: true });
	}
});
