...
test('formal guide editorial: guide URLs use Experience layout, category keeps Starlight', { timeout: 240_000 }, () => {
	...
	try {
		...
		const guideHtml = readFileSync(path.join(workspace, 'dist/example-game/example-guide/index.html'), 'utf8');
		...
		const categoryHtml = readFileSync(path.join(workspace, 'dist/example-game/gameplay/index.html'), 'utf8');
		...
		const hubHtml = readFileSync(path.join(workspace, 'dist/example-game/index.html'), 'utf8');
...
test('formal guide editorial: hero, facts, quickAnswer, evidence/sources optional', { timeout: 240_000 }, () => {
	...
	try {
		...
		// example-guide has full data: hero + facts + quick answer
		const guideHtml = readFileSync(path.join(workspace, 'dist/example-game/example-guide/index.html'), 'utf8');
		assert.match(guideHtml, /exp-guide-hero-image/, 'example-guide cover renders');
		...
		const locationHtml = readFileSync(path.join(workspace, 'dist/example-game/example-location/index.html'), 'utf8');
		assert.match(locationHtml, /exp-guide-hero-image/, 'example-location cover renders');
...
test('formal guide editorial: On This Page from H2s, numbering, and no duplicate nav', { timeout: 240_000 }, () => {
	...
	try {
		...
		const items = [...guideHtml.matchAll(/<li class="exp-on-this-page__item">/g)];
		assert.ok(items.length >= 3 && items.length <= 5, `On This Page should list 3-5 items, got ${items.length}`);
		assert.match(guideHtml, /exp-on-this-page__number">01</);
		assert.match(guideHtml, /exp-on-this-page__number">02</);
...
test('formal guide editorial: route context shows all routes; other routes excludes current', { timeout: 240_000 }, () => {
	...
	try {
		...
		assert.match(guideHtml, /href="\/example-game\/routes\/getting-started\//);
		assert.match(guideHtml, /href="\/example-game\/routes\/core-gameplay\//);
...
test('formal guide editorial: SEO + Pagefind intact after layout switch', { timeout: 240_000 }, () => {
	...
	try {
		...
		assert.match(
			guideHtml,
			/rel="canonical" href="https:\/\/example-game\.example\/example-game\/example-guide\//,
		);
		// Title unchanged
		...
		const guideMatches = sitemap.match(/example-game\/example-guide\//g) ?? [];
...
test('formal guide editorial: zh-CN chrome renders for guides', { timeout: 240_000 }, () => {
	...
	try {
		...
		assert.match(guideNav, /href="\/example-game\/"[^>]*>首页</);
		assert.match(guideNav, /href="\/example-game\/routes\/"[^>]*>路径</);
		assert.match(guideNav, /href="\/example-game\/guides\/"[^>]*>攻略</);
		assert.match(guideNav, /href="\/example-game\/#about"[^>]*>关于</);
