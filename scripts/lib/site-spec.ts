...
import { normalizeHubPath, pageHref, PathConfigError } from '../../src/lib/paths';
...
function parseSite(raw: unknown, location: string): SiteSpecSite {
	...
	const hubPathRaw = requireString(raw, 'hubPath', `${location}.hubPath`)!;
	try {
		normalizeHubPath(hubPathRaw);
	} catch (error) {
	...
	return {
		...
		siteUrl,
		hubPath: normalizeHubPath(hubPathRaw),
...
function parsePages(raw: unknown, location: string, categoryIds: Set<string>): SiteSpecPage[] {
	...
	return list.map((item, index) => {
		...
		if (!KEBAB.test(slug)) {
			fail('page slug must be a single kebab-case segment.', `pages[${index}].slug`, slug, `${loc}.slug`, 'Do not include hub prefixes.');
		}
		if (slugs.has(slug)) {
...
function parsePageEvidence(raw: unknown, location: string, pageIndex: number): SiteSpecPageEvidence[] {
	...
	return list.map((entry, evidenceIndex) => {
		...
		const sourceTypeRaw = requireString(entry, 'sourceType', `${loc}.sourceType`)!;
		if (!ALLOWED_EVIDENCE_SOURCE_TYPES.includes(sourceTypeRaw as EvidenceSourceType)) {
			fail(
...
function parseAnalytics(raw: unknown, location: string): SiteSpecAnalytics | undefined {
	if (raw === undefined || raw === null) return undefined;
	if (!isRecord(raw)) {
		fail('analytics must be a mapping.', 'analytics', raw, location, 'Use analytics.enabled and provider/measurementId.');
	}
	const enabled = requireBoolean(raw, 'enabled', location, { optional: true, defaultValue: false })!;
	if (!enabled) {
		return undefined;
	}
	const provider = requireString(raw, 'provider', `${location}.provider`);
...
function assertAnalyticsRequiresPrivacy(spec: SiteSpec) {
	...
	if (spec.analytics.provider !== 'ga4') return;
...
function parseAssets(raw: unknown, location: string): SiteSpecAsset[] {
	...
	return list.map((item, index) => {
		...
		return {
			...
			target,
			alt,
			sourceUrl: requireString(item, 'sourceUrl', `${loc}.sourceUrl`, { optional: true }),
