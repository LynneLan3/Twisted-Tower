...
function yamlQuote(value: string): string {
	if (/^-?(?:\d+\.?\d*|\.\d+)$/.test(value)) {
		// Numeric-looking strings must stay quoted, otherwise YAML round-trip
		// re-parses them as numbers and fails string-typed schemas.
	...
	for (const [key, value] of fields) {
			...
			for (const item of value) {
				if (item !== null && typeof item === 'object' && !Array.isArray(item)) {
					...
					for (const [childKey, childValue] of Object.entries(item as Record<string, unknown>)) {
						...
						lines.push(
							`    ${childKey}: ${typeof childValue === 'number' ? childValue : yamlQuote(String(childValue))}`,
						);
					}
					continue;
				}
		...
		if (typeof value === 'object') {
			...
			for (const [childKey, childValue] of Object.entries(value as Record<string, unknown>)) {
				if (childValue === undefined || childValue === null) continue;
				lines.push(`  ${childKey}: ${typeof childValue === 'number' ? childValue : yamlQuote(String(childValue))}`);
			}
			continue;
		}
	...
	if (config.pages && config.pages.length > 0) {
		...
		for (const page of config.pages) {
			...
			for (const item of page.evidence) {
				...
				lines.push(`\t\t\t\t\talt: ${tsString(item.alt)},`);
				if (item.caption) lines.push(`\t\t\t\t\tcaption: ${tsString(item.caption)},`);
				lines.push(`\t\t\t\t\tsourceType: ${tsString(item.sourceType)},`);
...
function buildPageMarkdown(spec: SiteSpec, pageId: string, rootDir: string, specDir: string): string {
	...
	if (cover) fields.push(['cover', `../../../assets/${cover.target}`]);
...
export function buildPlan(loaded: LoadedSiteSpec): GeneratorPlan {
	...
	if (spec.pages.length > 5) {
		warnings.push(
			`Spec declares ${spec.pages.length} pages. Skill guidance recommends 3–5 high-confidence pages for V1; review before expanding.`,
		);
	}
